...
// Networks - organized: MAINNETS (alphabetically), then TESTNETS
const NETWORKS = [
  // ============= MAINNETS (Alphabetical) =============
  {
    name: 'Arbitrum One',
    chainId: '0xa4b1',
    contractAddress: '0x9369D60FF784cC31311536baC9C7cd98A66cE235',
    rpcUrl: 'https://arb1.arbitrum.io/rpc',
    explorer: 'https://arbiscan.io/',
    buttonColor: '#28a0f0',
    logoUrl: 'img/arbitrum.png',
  ...
  },
  {
    ...
    contractAddress: '0xf47e08cf3513e2D3fc0625e106271004102C92c6',
  ...
  },
  {
    ...
    chainId: '0x38',
    contractAddress: '0xf98CbC628bF7Af83928efBE26AfC45BDc0c196E5', // TODO: Add contract address
    rpcUrl: 'https://bsc-dataseed1.binance.org',
    explorer: 'https://bscscan.com/',
    buttonColor: '#F3BA2F',
    logoUrl: 'img/bnb.png',
    nativeCurrency: { name: 'BNB', symbol: 'BNB', decimals: 18 },
    feeFunction: 'getGmFeeInBnb',
    isTestnet: false,
  },
  {
    name: 'Celo Mainnet',
    ...
    contractAddress: '0xaAC09b017F6E263f98F74699162d5921232856A7',
  ...
  },
  {
    name: 'Ink',
    chainId: '0xdef1',
    contractAddress: '0x480d9f49Be81d64b2A9F2c036Dd6d352753129FD', // TODO: Add contract address
    rpcUrl: 'https://rpc-gel.inkonchain.com',
    explorer: 'https://explorer.inkonchain.com/',
    buttonColor: '#6f00ffff',
    logoUrl: 'img/ink.png',
    nativeCurrency: { name: 'ETH', symbol: 'ETH', decimals: 18 },
    feeFunction: 'getGmFeeInEth',
    isTestnet: false,
  },
  {
    name: 'Mantle',
    chainId: '0x1388',
    contractAddress: '0x362DDD2e7515306b3F22f453D4bbCF3369853857', // TODO: Add contract address
    rpcUrl: 'https://rpc.mantle.xyz',
    explorer: 'https://explorer.mantle.xyz/',
    buttonColor: '#000000',
    logoUrl: 'img/mantle.png',
    nativeCurrency: { name: 'MNT', symbol: 'MNT', decimals: 18 },
    feeFunction: 'getGmFeeInMnt',
    isTestnet: false,
  },
  {
    name: 'Optimism',
    chainId: '0xa',
    contractAddress: '0x8c20C048a4e2D3d6d9A25DF7db23b4F0CBac52da', // TODO: Add contract address
  ...
  },
  {
    name: 'Soneium',
    chainId: '0x74c',
    contractAddress: '0xD6db4F5BBd1F83f5218bf2E4A78E7c4C609Ece9A',
  ...
  {
    name: 'Unichain',
    chainId: '0x82', // 130 decimal - Mainnet
    contractAddress: '0xad273a4384015be5a18345b595A9e6BBA2D9803C',
    rpcUrl: 'https://mainnet.unichain.org',
    fallbackRpc: 'https://unichain-rpc.publicnode.com',
    explorer: 'https://uniscan.xyz',
    buttonColor: '#FF007A',
    logoUrl: 'img/unichain.png',
    nativeCurrency: { name: 'ETH', symbol: 'ETH', decimals: 18 },
    feeFunction: 'getGmFeeInEth',
    isTestnet: false,
  },
  ...
  // ============= TESTNETS =============
  {
    ...
    contractAddress: '0xCdB9e8B10525B0c58b7ECCb3D2512EfB690c5f06',
...
const GM_ABI = [
  ...
  'function getGmFeeInBnb() view returns (uint256)',
  'function getGmFeeInMnt() view returns (uint256)',
  // ...existing code...
];
...
function renderNetworkUIOnce() { 
  ...
  if (mainnets.length > 0) {
    ...
    mainnetHeader.innerHTML = '<h2 class="section-header" data-section="mainnet">Mainnet Networks</h2>';
  ...
  if (testnets.length > 0) {
    ...
    testnetHeader.innerHTML = '<h2 class="section-header" data-section="testnet">Testnet Networks</h2>';
...
function renderNetworkCard(net) {
  ...
  container.innerHTML = `
    ...
    <div class="d-grid mb-2">
      <button class="sayGmBtn btn btn-secondary">Send GM ☀️</button>
    </div>
...
// ...existing code...
  ...
  // Auto-detect light colors and set dark text
  const hexToRgb = (hex) => {
  ...
  const rgb = hexToRgb(net.buttonColor);
  if (rgb) {
    const luminance = getLuminance(rgb.r, rgb.g, rgb.b);
    if (luminance > 0.5) {
      sayGmBtn.style.color = '#1a1a1a';
    }
  }
  ...
  // Check if contract address is placeholder (0x0000...) and disable button if so
  const isPlaceholderContract = net.contractAddress === '0x0000000000000000000000000000000000000000';
  if (isPlaceholderContract) {
    sayGmBtn.disabled = true;
    sayGmBtn.textContent = 'Coming Soon 🔜';
    sayGmBtn.style.opacity = '0.5';
    sayGmBtn.style.cursor = 'not-allowed';
    statusText.textContent = 'Contract not deployed yet';
    txStatus.textContent = 'This network will be available soon!';
  }
  ...
  sayGmBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      // Try up to 3 times with fallback RPC support
      let feeAttempts = 0;
      ...
      while (feeAttempts < maxFeeAttempts) {
        try {
          ...
          // Use fallback RPC on 2nd attempt if available
          let providerToUse = provider;
          if (feeAttempts === 2 && net.fallbackRpc) {
            debugLog(`[TRANSACTION] Trying fallback RPC: ${net.fallbackRpc}`);
            providerToUse = new ethers.JsonRpcProvider(net.fallbackRpc);
            const fallbackContract = new ethers.Contract(net.contractAddress, GM_ABI, providerToUse);
            const feePromise = fallbackContract[feeFunctionName]({ gasLimit: 100000 });
            const timeoutPromise = new Promise((_, reject) => 
              setTimeout(() => reject(new Error('Fee fetch timeout')), 30000)
            );
            feeWei = await Promise.race([feePromise, timeoutPromise]);
          } else {
            // Call the appropriate fee function based on network with explicit gas limit
            const feePromise = contract[feeFunctionName]({ gasLimit: 100000 });
            const timeoutPromise = new Promise((_, reject) => 
              setTimeout(() => reject(new Error('Fee fetch timeout')), 30000)
            );
            feeWei = await Promise.race([feePromise, timeoutPromise]);
          ...
          console.error(`[TRANSACTION] Fee fetch attempt ${feeAttempts} failed:`, feeError);
      ...
      if (tx && tx.hash) {
          ...
          try {
            ...
            if (receipt) {
              ...
              if (receipt.status === 1) {
                ...
                txStatus.innerHTML = 'Confirmed: <a href="' + explorerUrl + '" target="_blank" rel="noopener noreferrer" style="color: inherit; text-decoration: underline;">' + tx.hash + '</a>';
              } else {
                console.error('❌ [TRANSACTION] Transaction failed');
                statusText.textContent = 'Transaction failed ❌';
                const explorerUrl = net.explorer + 'tx/' + tx.hash;
                txStatus.innerHTML = 'Failed: <a href="' + explorerUrl + '" target="_blank" rel="noopener noreferrer" style="color: inherit; text-decoration: underline;">' + tx.hash + '</a>';
              }
              return true;
            }
            return false;
        ...
        if (!receiptFound) {
          ...
          const explorerUrl = net.explorer + 'tx/' + tx.hash;
          txStatus.innerHTML = 'Pending: <a href="' + explorerUrl + '" target="_blank" rel="noopener noreferrer" style="color: inherit; text-decoration: underline;">' + tx.hash + '</a>';
        }
      }
...
function initThemeToggle() {
  ...
  if (!themeToggle || !themeIcon) {
    debugWarn('Theme toggle elements not found');
    return;
  }
  ...
  window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', (e) => {
    if (!localStorage.getItem('theme')) {
      ...
      document.documentElement.setAttribute('data-theme', newTheme);
      themeIcon.textContent = newTheme === 'dark' ? '☀️' : '🌙';
      debugLog(`[THEME] System preference changed to ${newTheme} mode`);
    }
  });
}
